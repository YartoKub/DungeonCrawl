...
public static class Geo3D
{
    ...
    {   // Ётот кусочек вз€т из другого проекта, уже не помню зачем оно надо
    ...
    // ѕрименимо только к случайным группам точек. ћожет создавать полигоны, не €вл€ющиес€ выпуклыми
    public static List<Vector2> ArrangeCounterClockwise(List<Vector2> points)
    {
        ...
        sortedPoints.Sort((a, b) => { // ¬олшебна€ л€мбда функци€, никогда бы не додумалс€.
            float angleA = Mathf.Atan2(a.y - centroid.y, a.x - centroid.x);
            float angleB = Mathf.Atan2(b.y - centroid.y, b.x - centroid.x);
            return angleA.CompareTo(angleB); //  ажетс€ возвращает -1, 0, 1 а зависимости от результата сравнени€
        });
