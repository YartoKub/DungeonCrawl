...
public class PointManagerInspectorGUI: AbstractManagerEditor<PolygonManager>
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Multiple Overlap Test")) PolygonTestBox.AddOverlapDegenerates();
        if (GUILayout.Button("6 Start Classic Test")) PolygonTestBox.Add6Start();
        if (GUILayout.Button("Incorrect edge markings test")) PolygonTestBox.ReproducibleBadMarkingsTest();
        EditorGUILayout.LabelField("World Tests");
        if (GUILayout.Button("Purge World")) manager.PurgeWorld();
        if (GUILayout.Button("Regenerate World")) manager.RegenerateWorld();
        ...
        if (GUILayout.Button("Degenerate Collinnears")) PolygonTestBox.AddCollinnearDegenerates();
        if (GUILayout.Button("Two gems, not full overlap")) PolygonTestBox.GemCoveredUncovered();
        if (GUILayout.Button("Two gems, full overlap")) PolygonTestBox.FullOverlap(); 
        if (GUILayout.Button("Bantic Left/Right")) PolygonTestBox.BanticLeftRight();
        if (GUILayout.Button("Bantic Top/Bottom")) PolygonTestBox.BanticTopBottom();
        if (GUILayout.Button("Top Brick Bottom Bantic")) PolygonTestBox.TopBrickBottomBantic();
        if (GUILayout.Button("Two triangles, camping tent")) PolygonTestBox.TriangleAndSmallTriangleTwoOutsideCollinearsTent(); 
        if (GUILayout.Button("Two triangles, ice cream")) PolygonTestBox.TriangleAndSmallTriangleTwoInsideCollinearsIceCream();
        if (GUILayout.Button("Fill Hole Issue")) PolygonTestBox.CheckMarkAndTriangleHoleFilling(); 
        if (GUILayout.Button("Triangle inserted into square hole")) PolygonTestBox.SquareHoleHalfTriangle();
        EditorGUILayout.LabelField("Multitude Tests");
        if (GUILayout.Button("Half Square and square test")) PolygonTestBox.ThreeTriangleTest();
