...
public static class LipomaPolygonClipping
{
    ...
    public static GraphDynamicList GetGraph(CH2D_Chunk A, CH2D_Chunk B, int draw_connection = -1)
    {
        ...
        // (галочка) Этот шаг отличается от разметчика, но теперь надо просто сделать двустороннюю бродилку ищущую грани другого полигона и проводящую анализ их классификации
        // (галочка) Сложность этого шага зависит от доверия к входным данным. Если они корректны, то можно за O(N) проверить входную точку каждого эджа.
        // (галочка) Если доверия к ним нет, то O(N*2) с проверкой начальной и конечной точек. Если есть несогласие то возврат нулля.  
        // (галочка) Но лучше сделать проверку внутри вершин. Это ускорит проверку на множественных пересечениях. 
