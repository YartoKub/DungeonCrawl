...
public class PolygonTestBox : MonoBehaviour
{
    ...
    {   // ÷ентральна€ точка коллинеарн
        // тестирование возможности установлени€ корректной ориентации и пор€дка исход€щих граней относительно центральной точки.
        // Ёто чисто тестовый полигон, сам по себе он имеет некорректную иерархию и может привести к неточной классиифкации наружных граней.
        // ƒл€ проверки пор€дка граней на пересечении
        // size = x4, y3
    ...
    {
        Debug.Log("PolygonManager.GetManager().polygonAddMode был на секундочку переведен в режим FillHoles дл€ этого теста");
...
public class DegeneratePolygonTestEditor : Editor 
{
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Pizza Sectors"))
        {
            ...
            // ѕоследние четыре переверныты, они - дырки
            for (int i = 0; i < 4; i++)
            {
